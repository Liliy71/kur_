...
def analyze_file(filepath, virus):
    try:
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
            text = f.readlines()
            for i in text:
                if re.match(r".*mkdir %windir%\\.*", i):
                    virus[0] = 1
                if re.match(r".*attrib .*[%]*[w_][wb_]*[%]* +h", i):
                    virus[1] = 1
                if re.match(r".*echo.*>>[%]*[w_][wb_]*[%]*", i):
                    virus[2] = 1
                if re.match(r".*<%0>>[%]*[w_][wb_]*[%]*", i):
                    virus[3] = 1
                if re.match(r".*ren [%]*[w_][wb_]*[%]* *.bat", i):
                    virus[4] = 1
                if re.match(r".*copy [%]*[w_][wb_+.]* [%]*[w_][wb_+]*.*", i):
                    virus[5] = 1
                if re.match(r".*for [%]*[w_][wb_+.]* in (*.bat) do call [%]*[w_][wb_+.]*[%]* [%]*[w_][wb_+.]*[%]*", i):
                    virus[6] = 1
            return any(virus)
